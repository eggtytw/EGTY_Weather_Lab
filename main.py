...
MODEL_CONFIGS = {
    "FNV3": {
        ...
        "time_offset_h": 0,   # 相對 forecast_list.json 的時間偏移
    },
    "GENC": {
...
def active_main():
    ...
    if not fn_match or not lm_match:
        print("⚠️ 無法解析網頁數據。")
        return
    ...
    # --- 關鍵：初始化字典 ---
    active_list = {}
    output_path = "active_typhoon/active_list.json"
    ...
    if active_list:
        print(f"\n✨ 任務完成！共 {len(active_list)} 個氣旋數據。")
    else:
        print(f"\nℹ️ 24小時內無更新數據，已建立空的 JSON 檔案。")
